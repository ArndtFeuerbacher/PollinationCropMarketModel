...
*** This model variant tries to replicated Uwangabire and Gallai (2024)
...
BaseFoodIntake_pc(FAOsubreg,"ModelB_2010_scaled", cropgroup) = SUM((k, i)$(mapping_k_cropgroup(k,cropgroup) and map_i_FAOsubreg(i,FAOsubreg)),
                                        X02(i,k) * 10**3) / SUM(i$map_i_FAOsubreg(i,FAOsubreg), SubRegPop(i));

BaseTotalFoodIntake_pc(FAOsubreg,"ModelB_2010_scaled") = SUM((k,i)$(map_i_FAOsubreg(i,FAOsubreg)),X02(i,k)* 10**3) 
                                                    / SUM(i$map_i_FAOsubreg(i,FAOsubreg), SubRegPop(i));
                                                                                                     
BaseNutrientIntake_pc(FAOsubreg,"ModelB_2010_scaled", n) = SUM(i$map_i_FAOsubreg(i,FAOsubreg),SUM(k, nutritionmatrix(k,n) * X02(i,k)*10))
                                                    / SUM(i$map_i_FAOsubreg(i,FAOsubreg), SubRegPop(i)); 

value_shr_cropgroup(k,cropgroup, "ModelB_2010_scaled") = (SUM(i,PW.L(k)*Q.L(i,k)*mapping_k_cropgroup(k,cropgroup))) /
...
Solve ModelB_2010_scaled using MCP;
...
Budget(i,"ModelB_2010_scaled")  = R02(i);
...
resGlobal("GLobal_NetExports",k,sim)$sim2r(sim) = SUM(i, NX.L(i,k)); 
...
resGlobalperc_food("Global_NetExports",k,sim)$(sim2r(sim) and resGlobal("GLobal_NetExports",k,"Basemod2") )  = resGlobal("Global_NetExports",k,sim) / resGlobal("GLobal_NetExports",k,"Basemod2")-1;
...
resGlobalperc_foodtot("Global_NetExports",sim)$(sim2r(sim) and SUM(k, resGlobal("GLobal_NetExports",k,"Basemod2")))  = SUM(k,resGlobal("Global_NetExports",k,sim)) / SUM(k, resGlobal("GLobal_NetExports",k,"Basemod2"))-1;
...
resGlobalperc_croptot("Global_NetExports",sim)$(sim2r(sim) and SUM(k$kcrop(k), resGlobal("GLobal_NetExports",k,"Basemod2")))  = SUM(k$kcrop(k),resGlobal("Global_NetExports",k,sim)) / SUM(k$kcrop(k), resGlobal("GLobal_NetExports",k,"Basemod2"))-1;
...
resGlobalperc_pollcroptot("Global_NetExports",sim)$(sim2r(sim) and SUM(k$kpollcrop(k), resGlobal("GLobal_NetExports",k,"Basemod2")))  = SUM(k$kpollcrop(k),resGlobal("Global_NetExports",k,sim)) / SUM(k$kpollcrop(k), resGlobal("GLobal_NetExports",k,"Basemod2"))-1;

resGlobalPriceObs(k,"ModelB_2010_scaled") = 0;
*resGlobalPriceObs(k,"ModelB_2010_scaled")$(resPWperc(k,sim) GT 0 and sim2r(sim)) = 1;
resGlobalPriceObs(k,"ModelB_2010_scaled")$(PW02(k) and sim2r(sim)) = 1;

*resGlobalPrice(sim)$sim2r(sim) = SUM(k,resGlobalperc("World_Market_Price",k,sim)) / SUM(k,resGlobalPriceObs(k,"ModelB_2010_scaled"));

resGlobalFoodPrice_avg(sim)$(sim2r(sim) and SUM(k,resGlobalPriceObs(k,"ModelB_2010_scaled")))  = SUM(k,resGlobalperc_food("World_Market_Price",k,sim)) / SUM(k,resGlobalPriceObs(k,"ModelB_2010_scaled"));
resGlobalCropPrice_avg(sim)$(sim2r(sim) and SUM(k$kcrop(k),resGlobalPriceObs(k,"ModelB_2010_scaled")))  = SUM(k$kcrop(k),resGlobalperc_food("World_Market_Price",k,sim)) / SUM(k$kcrop(k),resGlobalPriceObs(k,"ModelB_2010_scaled"));               
resGlobalPollPrice_avg(sim)$(sim2r(sim) and SUM(k$kpollcrop(k),resGlobalPriceObs(k,"ModelB_2010_scaled"))) = SUM(k$kpollcrop(k),resGlobalperc_food("World_Market_Price",k,sim)) / SUM(k$kpollcrop(k),resGlobalPriceObs(k,"ModelB_2010_scaled"));  

* Change in crop group prices
resGlobalPriceObs_CropGroup(cropgroup,k,"ModelB_2010_scaled")$(PW.L(k) and sim2r(sim) and mapping_k_cropgroup(k,cropgroup)) = 1;

resGlobalPriceTotalObs_cropgroup(cropgroup,"ModelB_2010_scaled") = SUM(k$k2(k),resGlobalPriceObs_CropGroup(cropgroup,k,"ModelB_2010_scaled"));

resGlobalCropGroupPrice_avg(cropgroup,sim)$(sim2r(sim) and resGlobalPriceTotalObs_cropgroup(cropgroup,"ModelB_2010_scaled"))     =  SUM(k$mapping_k_cropgroup(k,cropgroup),resGlobalperc_food("World_Market_Price",k,sim)) /
*                                                                SUM(k,resGlobalPriceObs_CropGroup(cropgroup,k,"ModelB_2010_scaled"));
                                                                resGlobalPriceTotalObs_cropgroup(cropgroup,"ModelB_2010_scaled");
                                                                
resGlobalCropGroupPrice_wgt(cropgroup,sim)$sim2r(sim)  = SUM(k$mapping_k_cropgroup(k,cropgroup),
                                                resPWperc(k,sim) *value_shr_cropgroup(k,cropgroup, "ModelB_2010_scaled")) ;
...
resFoodDemand_reg(FAOsubreg,sim)$sim2r(sim)  = SUM(i$map_i_FAOsubreg(i,FAOsubreg),SUM(k, resX(i,k,sim))); 
resFoodDemand_reg_perc(FAOsubreg,sim)$(sim2r(sim) and resFoodDemand_reg(FAOsubreg,"Basemod2")) = resFoodDemand_reg(FAOsubreg,sim)/resFoodDemand_reg(FAOsubreg,"Basemod2") - 1;
 
resFoodDemandValue_reg(FAOsubreg,sim)$sim2r(sim)  = SUM(i$map_i_FAOsubreg(i,FAOsubreg),SUM(k, resX(i,k,sim)*resPW(k,sim))); 
resFoodDemandValue_reg_perc(FAOsubreg,sim)$(sim2r(sim) and resFoodDemandValue_reg(FAOsubreg,"Basemod2")) = resFoodDemandValue_reg(FAOsubreg,sim)/resFoodDemandValue_reg(FAOsubreg,"Basemod2") - 1;
