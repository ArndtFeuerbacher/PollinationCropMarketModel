...
onek4(k)$depratios(k,"Klein","mean") = 1;
...
* Calculate production value shares for k4 in the base
prodvalue_shr_base04(k)$k4(k) = SUM(i, Q04(i,k) * PW04(k)) / SUM((i,kp)$k4(k),Q04(i,kp) * PW04(kp));

prodvalue_shr_base04CHK= SUM(k,prodvalue_shr_base04(k));

ABORT $(abs(prodvalue_shr_base04CHK - 1) GT 0.000005) "Production base value shares in model C do not equal 1" ;
...
BaseFoodIntake_pc(FAOsubreg,"ModelC", cropgroup) = SUM((k, i)$(mapping_k_cropgroup(k,cropgroup) and map_i_FAOsubreg(i,FAOsubreg)),
                                        X04(i,k) * 10**3) / SUM(i$map_i_FAOsubreg(i,FAOsubreg), SubRegPop(i));

BaseTotalFoodIntake_pc(FAOsubreg,"ModelC") = SUM((k,i)$(map_i_FAOsubreg(i,FAOsubreg)),X04(i,k)* 10**3) 
                                                    / SUM(i$map_i_FAOsubreg(i,FAOsubreg), SubRegPop(i));
                                                                                                     
BaseNutrientIntake_pc(FAOsubreg,"ModelC", n) = SUM(i$map_i_FAOsubreg(i,FAOsubreg),SUM(k, nutritionmatrix(k,n) * X04(i,k)*10))
                                                    / SUM(i$map_i_FAOsubreg(i,FAOsubreg), SubRegPop(i)); 

value_shr_cropgroup(k,cropgroup, "ModelC") = (SUM(i,PW.L(k)*Q.L(i,k)*mapping_k_cropgroup(k,cropgroup))) /
...
resGlobal("Global_Production_wgt",k,sim)$sim4r(sim) = SUM(i, Q.L(i,k)* PW0(k)); 
resGlobal("GLobal_Demand_wgt",k,sim)$sim4r(sim)  = SUM(i, X.L(i,k)* PW0(k)); 
...
resGlobalperc_foodtot("Global_Production_wgt",sim)$(sim4r(sim) and SUM(k, resGlobal("Global_Production_wgt",k,"Basemod4")))  = SUM(k,resGlobal("Global_Production_wgt",k,sim))/SUM(k, resGlobal("Global_Production_wgt",k,"Basemod4"))-1;
resGlobalperc_foodtot("Global_Demand_wgt",sim)$(sim4r(sim) and SUM(k, resGlobal("Global_Demand_wgt",k,"Basemod4")))   = SUM(k, resGlobal("Global_Demand_wgt",k,sim)) / SUM(k, resGlobal("GLobal_Demand_wgt",k,"Basemod4"))-1;
...
resGlobalPriceObs(k,"ModelC") = 0;
*resGlobalPriceObs(k,"ModelC")$(resPWperc(k,sim) GT 0 and sim4r(sim)) = 1;
resGlobalPriceObs(k,"ModelC")$(PW04(k) and sim4r(sim)) = 1;

*resGlobalPrice(sim)$sim4r(sim) = SUM(k,resGlobalperc("World_Market_Price",k,sim)) / SUM(k,resGlobalPriceObs(k,"ModelC"));

resGlobalFoodPrice_avg(sim)$(sim4r(sim) and SUM(k,resGlobalPriceObs(k,"ModelC")))  = SUM(k,resGlobalperc_food("World_Market_Price",k,sim)) / SUM(k,resGlobalPriceObs(k,"ModelC"));
resGlobalCropPrice_avg(sim)$(sim4r(sim) and SUM(k$kcrop(k),resGlobalPriceObs(k,"ModelC")))  = SUM(k$kcrop(k),resGlobalperc_food("World_Market_Price",k,sim)) / SUM(k$kcrop(k),resGlobalPriceObs(k,"ModelC"));               
resGlobalPollPrice_avg(sim)$(sim4r(sim) and SUM(k$kpollcrop(k),resGlobalPriceObs(k,"ModelC"))) = SUM(k$kpollcrop(k),resGlobalperc_food("World_Market_Price",k,sim)) / SUM(k$kpollcrop(k),resGlobalPriceObs(k,"ModelC"));  

* Change in crop group prices
resGlobalPriceObs_CropGroup(cropgroup,k,"ModelC")$(PW.L(k) and sim4r(sim) and mapping_k_cropgroup(k,cropgroup)) = 1;

resGlobalPriceTotalObs_cropgroup(cropgroup,"ModelC") = SUM(k$k4(k),resGlobalPriceObs_CropGroup(cropgroup,k,"ModelC"));

resGlobalCropGroupPrice_avg(cropgroup,sim)$(sim4r(sim) and resGlobalPriceTotalObs_cropgroup(cropgroup,"ModelC"))    =  SUM(k$mapping_k_cropgroup(k,cropgroup),resGlobalperc_food("World_Market_Price",k,sim)) /
*                                                                SUM(k,resGlobalPriceObs_CropGroup(cropgroup,k,"ModelC"));
                                                                resGlobalPriceTotalObs_cropgroup(cropgroup,"ModelC");

resGlobalCropGroupPrice_wgt(cropgroup,sim)$sim4r(sim)  = SUM(k$mapping_k_cropgroup(k,cropgroup),
                                                resPWperc(k,sim) *value_shr_cropgroup(k,cropgroup, "ModelC")) ;
...
resFoodDemand_reg(FAOsubreg,k,sim)$sim4r(sim)  = SUM(i$map_i_FAOsubreg(i,FAOsubreg), resX(i,k,sim));  
resFoodDemand_reg_perc(FAOsubreg,k,sim)$(sim4r(sim) and resFoodDemand_reg(FAOsubreg,k,"Basemod4"))  = SUM(i$map_i_FAOsubreg(i,FAOsubreg), resX(i,k,sim)) / resFoodDemand_reg(FAOsubreg,k,"Basemod4") -1;  

resFoodDemand_macroreg_cropgroup(reg,cropgroup,sim)$sim4r(sim)  = SUM(FAOsubreg, map_reg_FAOsubreg(reg,FAOsubreg) * SUM(k, mapping_k_cropgroup(k,cropgroup) * resFoodDemand_reg(FAOsubreg,k,sim)) );
*Calculate global changes
resFoodDemand_macroreg_cropgroup("World",cropgroup,sim)$sim4r(sim)  = SUM(reg,resFoodDemand_macroreg_cropgroup(reg,cropgroup,sim));
** calculate percentage changes
resFoodDemand_macroreg_cropgroup_perc(reg,cropgroup,sim)$(sim4r(sim) and resFoodDemand_macroreg_cropgroup(reg,cropgroup,"Basemod4")) =   resFoodDemand_macroreg_cropgroup(reg,cropgroup,sim)/resFoodDemand_macroreg_cropgroup(reg,cropgroup,"Basemod4") -1 ;

          
resTotalFoodDemand_reg(FAOsubreg,sim)$sim4r(sim)  = SUM(i$map_i_FAOsubreg(i,FAOsubreg),SUM(k, resX(i,k,sim))); 
resTotalFoodDemand_reg_perc(FAOsubreg,sim)$(sim4r(sim) and resTotalFoodDemand_reg(FAOsubreg,"Basemod4")) = resTotalFoodDemand_reg(FAOsubreg,sim)/resTotalFoodDemand_reg(FAOsubreg,"Basemod4") - 1;
      
resTotalFoodDemandValue_reg(FAOsubreg,sim)$sim4r(sim)  = SUM(i$map_i_FAOsubreg(i,FAOsubreg),SUM(k, resX(i,k,sim)*resPW(k,sim))); 
resTotalFoodDemandValue_reg_perc(FAOsubreg,sim)$(sim4r(sim) and resTotalFoodDemandValue_reg(FAOsubreg,"Basemod4")) = resTotalFoodDemandValue_reg(FAOsubreg,sim)/resTotalFoodDemandValue_reg(FAOsubreg,"Basemod4") - 1;
