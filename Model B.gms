...
*** This model extends the Uwangabire and Gallai (2023) study
...
onek3(k)$depratios(k,"Klein","mean") = 1;
...
* Calculate production value shares for k3 in the base
prodvalue_shr_base03(k)$k3(k) = SUM(i, Q03(i,k) * PW03(k)) / SUM((i,kp)$k3(k),Q03(i,kp) * PW03(kp));

prodvalue_shr_base03CHK= SUM(k,prodvalue_shr_base03(k));

ABORT $(abs(prodvalue_shr_base03CHK - 1) GT 0.000005) "Production base value shares in model B  do not equal 1" ;
...
BaseFoodIntake_pc(FAOsubreg,"ModelB", cropgroup) = SUM((k, i)$(mapping_k_cropgroup(k,cropgroup) and map_i_FAOsubreg(i,FAOsubreg)),
                                        X03(i,k) * 10**3) / SUM(i$map_i_FAOsubreg(i,FAOsubreg), SubRegPop(i));

BaseTotalFoodIntake_pc(FAOsubreg,"ModelB") = SUM((k,i)$(map_i_FAOsubreg(i,FAOsubreg)),X03(i,k)* 10**3) 
                                                    / SUM(i$map_i_FAOsubreg(i,FAOsubreg), SubRegPop(i));
                                                                                                     
BaseNutrientIntake_pc(FAOsubreg,"ModelB", n) = SUM(i$map_i_FAOsubreg(i,FAOsubreg),SUM(k, nutritionmatrix(k,n) * X03(i,k)*10))
                                                    / SUM(i$map_i_FAOsubreg(i,FAOsubreg), SubRegPop(i)); 

value_shr_cropgroup(k,cropgroup, "ModelB") = (SUM(i,PW.L(k)*Q.L(i,k)*mapping_k_cropgroup(k,cropgroup))) /
...
resGlobal("Global_Production_wgt",k,sim)$sim3r(sim) = SUM(i, Q.L(i,k)* PW0(k)); 
resGlobal("GLobal_Demand_wgt",k,sim)$sim3r(sim)  = SUM(i, X.L(i,k)* PW0(k)); 
...
resGlobalperc_foodtot("Global_Production_wgt",sim)$(sim3r(sim) and SUM(k, resGlobal("Global_Production_wgt",k,"Basemod3")))  = SUM(k,resGlobal("Global_Production_wgt",k,sim))/SUM(k, resGlobal("Global_Production_wgt",k,"Basemod3"))-1;
resGlobalperc_foodtot("Global_Demand_wgt",sim)$(sim3r(sim) and SUM(k, resGlobal("Global_Demand_wgt",k,"Basemod3")))   = SUM(k, resGlobal("Global_Demand_wgt",k,sim)) / SUM(k, resGlobal("GLobal_Demand_wgt",k,"Basemod3"))-1;
...
resGlobalPriceObs(k,"ModelB")$(not k3(k)) = 0;
*resGlobalPriceObs(k,"ModelB")$(resPWperc(k,sim) GT 0 and sim3r(sim)) = 1;
resGlobalPriceObs(k,"ModelB")$(PW03(k) and sim3r(sim)) = 1;

*resGlobalPrice(sim)$sim3r(sim) = SUM(k,resGlobalperc("World_Market_Price",k,sim)) / SUM(k,resGlobalPriceObs(k,"ModelB"));

resGlobalFoodPrice_avg(sim)$(sim3r(sim) and SUM(k,resGlobalPriceObs(k,"ModelB")))  = SUM(k,resGlobalperc_food("World_Market_Price",k,sim)) / SUM(k,resGlobalPriceObs(k,"ModelB"));
resGlobalCropPrice_avg(sim)$(sim3r(sim) and SUM(k$kcrop(k),resGlobalPriceObs(k,"ModelB")))  = SUM(k$kcrop(k),resGlobalperc_food("World_Market_Price",k,sim)) / SUM(k$kcrop(k),resGlobalPriceObs(k,"ModelB"));               
resGlobalPollPrice_avg(sim)$(sim3r(sim) and SUM(k$kpollcrop(k),resGlobalPriceObs(k,"ModelB"))) = SUM(k$kpollcrop(k),resGlobalperc_food("World_Market_Price",k,sim)) / SUM(k$kpollcrop(k),resGlobalPriceObs(k,"ModelB"));  

* Change in crop group prices
resGlobalPriceObs_CropGroup(cropgroup,k,"ModelB")$(PW.L(k) and sim3r(sim) and mapping_k_cropgroup(k,cropgroup)) = 1;

resGlobalPriceTotalObs_cropgroup(cropgroup,"ModelB") = SUM(k$k3(k),resGlobalPriceObs_CropGroup(cropgroup,k,"ModelB"));

resGlobalCropGroupPrice_avg(cropgroup,sim)$(sim3r(sim) and resGlobalPriceTotalObs_cropgroup(cropgroup,"ModelB"))     =  SUM(k$mapping_k_cropgroup(k,cropgroup),resGlobalperc_food("World_Market_Price",k,sim)) /
*                                                                SUM(k,resGlobalPriceObs_CropGroup(cropgroup,k,"ModelB"));
                                                                resGlobalPriceTotalObs_cropgroup(cropgroup,"ModelB");

resGlobalCropGroupPrice_wgt(cropgroup,sim)$sim3r(sim)  = SUM(k$mapping_k_cropgroup(k,cropgroup),
                                                resPWperc(k,sim) *value_shr_cropgroup(k,cropgroup, "ModelB")) ;
...
resFoodDemand_reg(FAOsubreg,k,sim)$sim3r(sim)  = SUM(i$map_i_FAOsubreg(i,FAOsubreg), resX(i,k,sim));  
resFoodDemand_reg_perc(FAOsubreg,k,sim)$(sim3r(sim) and resFoodDemand_reg(FAOsubreg,k,"Basemod3"))  = SUM(i$map_i_FAOsubreg(i,FAOsubreg), resX(i,k,sim)) / resFoodDemand_reg(FAOsubreg,k,"Basemod3") -1;  

resFoodDemand_macroreg_cropgroup(reg,cropgroup,sim)$sim3r(sim)  = SUM(FAOsubreg, map_reg_FAOsubreg(reg,FAOsubreg) * SUM(k, mapping_k_cropgroup(k,cropgroup) * resFoodDemand_reg(FAOsubreg,k,sim)) );
*Calculate global changes
resFoodDemand_macroreg_cropgroup("World",cropgroup,sim)$sim3r(sim)  = SUM(reg,resFoodDemand_macroreg_cropgroup(reg,cropgroup,sim));
** calculate percentage changes
resFoodDemand_macroreg_cropgroup_perc(reg,cropgroup,sim)$(sim3r(sim) and resFoodDemand_macroreg_cropgroup(reg,cropgroup,"Basemod3")) =   resFoodDemand_macroreg_cropgroup(reg,cropgroup,sim)/resFoodDemand_macroreg_cropgroup(reg,cropgroup,"Basemod3") -1 ;

     
resTotalFoodDemand_reg(FAOsubreg,sim)$sim3r(sim)  = SUM(i$map_i_FAOsubreg(i,FAOsubreg),SUM(k, resX(i,k,sim))); 
resTotalFoodDemand_reg_perc(FAOsubreg,sim)$(sim3r(sim) and resTotalFoodDemand_reg(FAOsubreg,"Basemod3")) = resTotalFoodDemand_reg(FAOsubreg,sim)/resTotalFoodDemand_reg(FAOsubreg,"Basemod3") - 1;
         
resTotalFoodDemandValue_reg(FAOsubreg,sim)$sim3r(sim)  = SUM(i$map_i_FAOsubreg(i,FAOsubreg),SUM(k, resX(i,k,sim)*resPW(k,sim))); 
resTotalFoodDemandValue_reg_perc(FAOsubreg,sim)$(sim3r(sim) and resTotalFoodDemandValue_reg(FAOsubreg,"Basemod3")) = resTotalFoodDemandValue_reg(FAOsubreg,sim)/resTotalFoodDemandValue_reg(FAOsubreg,"Basemod3") - 1;
